...
- 基本面 : 比特幣收盤價及收盤價、高低點、每小時交易量、

`                           `收益率和對數收益率**(Log Return)**
...
1. **LSTM**
1. 輸入輸出維度
   1) 168 – 1 : 前一個禮拜的資料預測下一個小時的價格
   1) 168 – 24 : 前一個禮拜的資料預測下一天的價格
   1) 168 – 168 : 前一個禮拜的資料預測下一個禮拜的價格
   1) 672 – 168 : 前四個禮拜的資料預測下一個禮拜的價格
1. 時間推移
   1) 每小時
   1) 每八小時
   1) 每天
...
根據資料特性做不同的處理，去極值、標準化，像是SOPR就應做去噪     			處理
